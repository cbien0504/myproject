...
def upsert_master_from_tgdd(
    ...
    ).whenMatchedUpdate(set=column_mapping).whenNotMatchedInsertAll().execute()
    master_columns = [col.name for col in master_table.toDF().schema.fields]
    if "uuid" not in master_columns:
        master_table = master_table.toDF().withColumn("uuid", F.lit(None).cast("string"))
